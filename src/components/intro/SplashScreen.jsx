...
export default function SplashScreen() {
  const { brand, finishSplash, splashDuration } = useBrand();
  const { speak } = useVoice();
  const isApple = brand === "apple";
  const info = brands[brand] ?? brands.pc;
  ...
  // Saludo corporativo cálido y genérico, disparado una única vez al montar la intro.
  useEffect(() => {
    const greeting = window.setTimeout(
      () => speak("¡Hola! Qué bueno tenerte por acá. En un instante te mostramos todo."),
      350
    );
    ...
    >
      ...
      <div
        className={`absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full opacity-30 blur-2xl sm:h-[36rem] sm:w-[36rem] sm:blur-3xl ${
          isApple ? "bg-[radial-gradient(circle,var(--color-apple-500)_0%,transparent_70%)]" : ""
        }`}
        style={
          isApple
            ? undefined
      ...
      >
        <div className="relative">
          <motion.div
            ...
            className={`absolute -inset-6 rounded-[2rem] blur-2xl ${
              isApple ? "bg-apple-500/20" : "bg-brand-500/20"
            }`}
          />
          {isApple ? (
            <div className="relative flex h-16 w-16 items-center justify-center rounded-[1.5rem] border border-apple-500/30 bg-surface sm:h-24 sm:w-24">
              <AppleMark className="h-9 w-9 text-apple-200 sm:h-12 sm:w-12" />
            </div>
          ) : (
        ...
        >
          ...
          <h1 className="font-display text-2xl font-semibold tracking-tight text-ink sm:text-4xl">
            Silicon<span className={info.gradientClass}>{info.name.replace("Silicon", "")}</span>
          </h1>
        </motion.div>
        ...
        <motion.div
          ...
          className={`h-px w-32 origin-center bg-gradient-to-r from-transparent to-transparent sm:w-40 ${
            isApple ? "via-apple-400/70" : "via-brand-400/70"
          }`}
        />
      </motion.div>
      ...
      >
        <span
          className={`h-1.5 w-1.5 rounded-full animate-pulse-slow ${
            isApple ? "bg-apple-400" : "bg-brand-400"
          }`}
        />
