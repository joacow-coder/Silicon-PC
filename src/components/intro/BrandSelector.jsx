...
const options = [
  {
    ...
    greeting: "Bienvenido a Silicon PC.",
  },
  {
    key: "apple",
    icon: AppleMark,
    ...
    greeting: "Bienvenido a Silicon Apple.",
  },
];
...
export default function BrandSelector() {
  ...
  const handleSelect = (option) => {
    speak(option.greeting);
    chooseBrand(option.key);
  };
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-y-auto overflow-x-hidden bg-bg px-4 py-14 sm:px-5 sm:py-16"
    >
      ...
      <motion.div
        ...
        className="relative mb-8 flex flex-col items-center gap-3 text-center sm:mb-14 sm:gap-4"
      >
        <Logo className="h-11 w-11 sm:h-14 sm:w-14" />
        <div>
          <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-ink-faint sm:text-xs sm:tracking-[0.3em]">
            Elegí tu experiencia
          </span>
          <h2 className="mt-2 font-display text-xl font-semibold tracking-tight text-ink sm:mt-3 sm:text-3xl">
        ...
        {options.map((option, i) => {
          ...
          return (
            <motion.button
              ...
              whileHover={{ y: -4 }}
              whileTap={{ scale: 0.98 }}
              className={`group relative flex min-h-[44px] flex-col items-start gap-4 overflow-hidden rounded-3xl border border-border bg-surface p-6 text-left transition-colors sm:gap-5 sm:p-8 ${option.ring}`}
            >
              <div
              ...
              <div className="relative inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-border bg-surface-2 sm:h-12 sm:w-12">
                <Icon className={`h-5 w-5 sm:h-6 sm:w-6 ${option.accentText}`} />
              </div>
