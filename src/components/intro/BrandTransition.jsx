...
export default function BrandTransition() {
  ...
  const Icon = pendingBrand === "apple" ? AppleMark : Cpu;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Icon className={pendingBrand === "apple" ? "h-6 w-6 text-apple-400 sm:h-7 sm:w-7" : "h-6 w-6 text-brand-400 sm:h-7 sm:w-7"} />
        </motion.div>
        <motion.p
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.2 }}
          className="text-center font-display text-base font-medium text-ink-muted sm:text-lg"
        >
          Entrando a{" "}
