...
import { appleServices } from "../../data/appleServices";
  ...
  return (
    <section id="servicios" className="relative py-20 sm:py-28">
      <Container className="flex flex-col gap-12">
        <SectionHeading
          eyebrow="Service técnico Apple"
          title="Reparación de iPhone con procedimientos de marca"
          description="Trabajamos con protocolos específicos del ecosistema Apple y repuestos verificados, con total transparencia sobre lo que se hace en tu equipo."
        />

        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {appleServices.map((service, i) => {
                  ...
                  <ul className="mt-1 flex flex-col gap-2">
                    {service.points.map((point) => (
                      <li key={point} className="flex items-center gap-2 text-xs text-ink-muted">
                        <Check className="h-3.5 w-3.5 text-apple-300 shrink-0" />
                        {point}
                      </li>
                    ))}
                  </ul>
